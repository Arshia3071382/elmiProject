...
import { Trophy, Rocket, ShieldAlert } from "lucide-react";
...
export default function RankRadarCard({
  ...
  const higherName = higherStudent?.name || "رتبه بالاتر";
  const higherScore = higherStudent?.score || score + 18;

  const lowerName = lowerStudent?.name || "رتبه پایین‌تر";
  const lowerScore = lowerStudent?.score || Math.max(0, score - 12);

  const diffToHigher = higherStudent ? Math.max(0, higherStudent.score - score) : 18;
  const diffToLower = lowerStudent ? Math.max(0, score - lowerStudent.score) : 12;
  ...
  return (
    ...
    >
      ...
      {/* ساختار ریسپانسیو: در موبایل عمودی (بالاتر، شما، پایین‌تر) و در دسکتاپ افقی */}
      ...
      {/* بخش پایین: کارت‌های اطلاعات فاصله */}
