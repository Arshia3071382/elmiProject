...
export default function AppPreviewPage() {
  ...
  // اصلاح ۱: خواندن آنی وضعیت لاگین از لوکال استوریج در همان رندر اولیه برای جلوگیری از پرش
  const [isLoggedIn, setIsLoggedIn] = useState(() => {
    if (typeof window !== 'undefined') {
      return Boolean(localStorage.getItem('studentPhone') || localStorage.getItem('studentNationalId') || localStorage.getItem('studentToken') || localStorage.getItem('token'))
    }
    return false
  })
  ...
  const handleLogout = async () => {
    ...
    // پاک کردن لوکال استوریج برای هماهنگی کامل
